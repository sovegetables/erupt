...
@Setter
@Entity
@Table(name = "mes_inspection_record_detail")
@Erupt(name = "检验记录详情")
public class MesInspectionRecordDetail extends BaseModel {
    ...
    @EruptField(
            edit = @Edit(title = "产品序号", numberType = @NumberType(min = 1), show = false)
    )
    private Long serialNumber;

    @EruptField(
            views = @View(title = "结果"),
            edit = @Edit(title = "结果", boolType = @BoolType(trueText = "合格", falseText = "不合格"), notNull = true)
    )
