...
@Getter
@Entity
@Table(name = "mes_other_out")
@Erupt(name = "其他出库",
        power = @Power(delete = false),
//        dataProxy = MesOtherOutService.class,
        filter = @Filter(
                value = "MesOtherOut.tenantId",
                conditionHandler = TenantFilter.class))
public class MesOtherOut extends TenantMetaModel {
    ...
    @JoinColumn(name = "mes_other_out_id")
    @OneToMany(cascade = CascadeType.ALL, orphanRemoval = true)
    @EruptField(
            views = @View(title = "详情"),
            edit = @Edit(title = "详情", type = EditType.TAB_TABLE_ADD)
    )
    private List<MesOtherOutDetail> mesOtherOutDetails;
}
